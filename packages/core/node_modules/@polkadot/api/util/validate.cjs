...
function doDoubleMap(registry, creator, args) {
  ...
  (0, _util.assert)(args.length === 2, () => `${sig(registry, creator, [key1, key2])} is a double map, requiring 2 arguments, ${args.length} found`); // pass as tuple
...
function doMap(registry, creator, args) {
  ...
  (0, _util.assert)(args.length === 1, () => `${sig(registry, creator, [key])} is a map, requiring 1 argument, ${args.length} found`); // expand
...
function doNMap(registry, creator, args) {
  ...
  (0, _util.assert)(args.length === keyVec.length, () => `${sig(registry, creator, keyVec)} is a multi map, requiring ${keyVec.length} arguments, ${args.length} found`); // pass as tuple
...
function extractStorageArgs(registry, creator, _args) {
  ...
  (0, _util.assert)(args.length === 0, () => `${sig(registry, creator, [])} does not take any arguments, ${args.length} found`); // no args
